...
| | |
|---|---|
| **Live** | `3889fc5`, tagged **`deploy-2026-09-15`** — #30: inbound email attachments actually arrive. The gates that lied (isInline on real files, hasAttachments false for inline-only, MIME-inflated size against the raw cap, an unpaged listing) are gone; the listing is fetched for every filed message and DecideInboundFile holds the rules, pinned by 16 tests. Verified against production data, not just probes: the first inbound attachment ever recorded landed on lead #415 at 04:59Z on deploy day. API-only release — bundle hash unchanged from `deploy-2026-09-11` (`index-fENx4aL9.js`), snapshots untouched, no migrations. |
| **`production` branch** | `3889fc5` = live. |
