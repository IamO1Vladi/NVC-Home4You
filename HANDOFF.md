...
| | |
|---|---|
| **Live** | `08858ad`, tagged **`deploy-2026-09-05`** — the twelve new B/C floor-plan renders (also serving the window-placement stage; A-series untouched) and the C5/C6 correction: 5 and 6 спални in the subtitles, door counts following one-per-bedroom, so those configurations price one interior door higher than before, correctly. Probe-verified: served bundle `index-BO4E-7vk.js` = the commit's build, and the live plan-C5.webp is byte-identical to the converted file. |
| **`production` branch** | `08858ad` = live. |
| **`master`** | = `production`, plus this note. Next up: #9, per-breakpoint srcset. |
