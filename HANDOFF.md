...
| | |
|---|---|
| **Live** | `e639791`, tagged **`deploy-2026-09-11`** — #9 shipped (`/api/img?w=` with the 13-rung width ladder + the SPA srcset helper, so phones stop downloading 2560px originals), the A37 terrace rule, the reply fix ("sent but not recorded" now says so instead of inviting the duplicate-send that hit lead #334 on 2026-09-11 — see the SentNotRecorded outcome), the `import-payments-sheet` CLI verb (already run against prod 2026-09-10: 15 customers, 4 factories), and #29 settled at 20 MB. Probe-verified: served bundle `index-fENx4aL9.js` = the commit's build, prerender 52/52 on the same hash, `?w=320` answers 20.5 KB where the original is 188 KB, admin API answers 401 closed. No migrations shipped (prod schema already at `AddActivityRecipients`). |
| **`production` branch** | `e639791` = live. |
