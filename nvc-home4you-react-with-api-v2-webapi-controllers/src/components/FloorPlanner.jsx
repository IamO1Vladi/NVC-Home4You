...
const FINISHES = [
  { key: 'wood', label: 'Wood', tex: 'flooring/wood.png', tileM: 0.62, base: '#7a4f2b', overlay: 'planks', rot: 0 },
  { key: 'laminate', label: 'Laminate', tex: 'flooring/laminate.png', tileM: 0.55, base: '#6b4a2f', overlay: 'planks', rot: 90 },
  { key: 'spc', label: 'SPC', tex: 'flooring/spc.png', tileM: 0.48, base: '#635b54', overlay: 'planks', rot: 0 },
  { key: 'tile', label: 'Tile', tex: 'flooring/tile.png', tileM: 0.36, base: '#d1d5db', overlay: 'grid', rot: 0 },
  { key: 'pvc', label: 'PVC', tex: 'flooring/pvc.png', tileM: 0.44, base: '#cbd5e1', overlay: 'grid', rot: 0 },
  { key: 'concrete', label: 'Concrete', tex: 'flooring/concrete.png', tileM: 0.95, base: '#6b7280', overlay: 'speckle', rot: 0 },
]
...
export default function FloorPlanner() {
  ...
  const onLayoutFileChange = async (ev) => {
    ...
    try {
      ...
      console.error(err)
      const msg =
        typeof t('planner.loadLayoutError') === 'string'
          ? t('planner.loadLayoutError')
  ...
  const showT = typeof t('planner.show') === 'string' ? t('planner.show') : 'Show'
  const hideT = typeof t('planner.hide') === 'string' ? t('planner.hide') : 'Hide'
  const zoomT = typeof t('planner.zoom') === 'string' ? t('planner.zoom') : 'Zoom'
  const legendSnapTitle =
    typeof t('planner.legendSnapTitle') === 'string' ? t('planner.legendSnapTitle') : 'Snapping & multi-place'
  ...
  const onT = typeof t('planner.on') === 'string' ? t('planner.on') : 'ON'
  const offT = typeof t('planner.off') === 'string' ? t('planner.off') : 'OFF'
  const downloadLayoutT = typeof t('planner.downloadLayout') === 'string' ? t('planner.downloadLayout') : 'Download layout'
  const loadLayoutT = typeof t('planner.loadLayout') === 'string' ? t('planner.loadLayout') : 'Load layout'

  const legendSnapT = t('planner.legendSnap')
  ...
  const snapStatusT = t('planner.snapStatus')
      ...
      <div className="fp-topbar">
        ...
        <div className="fp-top-actions">
          <button className="btn ghost" onClick={undo} disabled={!canUndo} title="Ctrl/⌘+Z">
            {typeof t('planner.undo') === 'string' ? t('planner.undo') : 'Undo'}
          </button>
          <button className="btn ghost" onClick={redo} disabled={!canRedo} title="Ctrl/⌘+Y or Ctrl/⌘+Shift+Z">
            {typeof t('planner.redo') === 'string' ? t('planner.redo') : 'Redo'}
          </button>
      ...
      >
        <summary className="fp-legend-sum">
          <span style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
            <span className="fp-legend-title">
              {typeof t('planner.legend') === 'string' ? t('planner.legend') : 'Legend / How to use'}
            </span>
            <span className="fp-legend-hint">
              {typeof t('planner.legendHint') === 'string' ? t('planner.legendHint') : 'Quick guide + shortcuts'}
            </span>
          </span>
        ...
        <div className="fp-legend-grid">
          <div className="fp-legend-item">
            <div className="fp-legend-k">🧱 {typeof t('planner.tool.room') === 'string' ? t('planner.tool.room') : 'Room'}</div>
            <div className="fp-legend-v">
              {typeof t('planner.legendRoom') === 'string'
                ? t('planner.legendRoom')
                : 'Drag to draw a room. Rooms cannot overlap; choose type + floor finish from the panel.'}
            </div>
          </div>

          <div className="fp-legend-item">
            <div className="fp-legend-k">🧱 {typeof t('planner.tool.wall') === 'string' ? t('planner.tool.wall') : 'Wall'}</div>
            <div className="fp-legend-v">
              {typeof t('planner.legendWall') === 'string'
                ? t('planner.legendWall')
                : 'Drag to draw an interior wall segment (auto axis-aligned). Doors snap to any wall; windows snap to exterior walls only.'}
            </div>
          </div>

          <div className="fp-legend-item">
            <div className="fp-legend-k">🚪 / 🪟 {typeof t('planner.openings') === 'string' ? t('planner.openings') : 'Doors & windows'}</div>
            <div className="fp-legend-v">
              {typeof t('planner.legendOpenings') === 'string'
                ? t('planner.legendOpenings')
                : 'Click to place. Switch to Select to drag along a wall. Doors support Flip swing / hinge in the Inspector.'}
            </div>
          </div>

          <div className="fp-legend-item">
            <div className="fp-legend-k">🖱️ {typeof t('planner.tool.select') === 'string' ? t('planner.tool.select') : 'Select'}</div>
            <div className="fp-legend-v">
              {typeof t('planner.legendSelect') === 'string'
                ? t('planner.legendSelect')
                : 'Click an object to inspect it. Drag rooms to move (with snapping + no-overlap). Drag doors/windows to reposition.'}
            </div>
          </div>

          <div className="fp-legend-item">
            <div className="fp-legend-k">🧭 {typeof t('planner.tool.pan') === 'string' ? t('planner.tool.pan') : 'Pan'} / {zoomT}</div>
            <div className="fp-legend-v">
              {typeof t('planner.legendPan') === 'string'
                ? t('planner.legendPan')
        ...
        <div className="fp-legend-keys">
          <div className="fp-keyrow">
            ...
            <span className="fp-keytext">{typeof t('planner.keyAlt') === 'string' ? t('planner.keyAlt') : 'Disable snapping while dragging'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">SHIFT</span>
            <span className="fp-keytext">
              {typeof t('planner.keyShift') === 'string' ? t('planner.keyShift') : 'Constrain axis while moving rooms'}
            </span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">ESC</span>
            <span className="fp-keytext">{typeof t('planner.keyEsc') === 'string' ? t('planner.keyEsc') : 'Release canvas capture (click canvas to capture again)'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">V</span>
            <span className="fp-keytext">{typeof t('planner.keyV') === 'string' ? t('planner.keyV') : 'Select tool'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">R</span>
            <span className="fp-keytext">{typeof t('planner.keyR') === 'string' ? t('planner.keyR') : 'Room tool'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">W</span>
            <span className="fp-keytext">{typeof t('planner.keyW') === 'string' ? t('planner.keyW') : 'Wall tool'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">D</span>
            <span className="fp-keytext">{typeof t('planner.keyD') === 'string' ? t('planner.keyD') : 'Door tool'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">N</span>
            <span className="fp-keytext">{typeof t('planner.keyN') === 'string' ? t('planner.keyN') : 'Window tool'}</span>
          </div>
          <div className="fp-keyrow">
            <span className="fp-key">P</span>
            <span className="fp-keytext">{typeof t('planner.keyP') === 'string' ? t('planner.keyP') : 'Pan tool'}</span>
          </div>
        </div>
      </details>
          ...
          <div className="fp-section">
            ...
            <div className="fp-toolopts">
              <label className="fp-check">
                ...
                <span>{typeof t('planner.multiPlace') === 'string' ? t('planner.multiPlace') : 'Multi-place'}</span>
              </label>
              <div className="fp-muted">
                {typeof t('planner.multiPlaceHint') === 'string'
                  ? t('planner.multiPlaceHint')
          ...
          <div className="fp-section">
            ...
            <div className="fp-toolopts">
              <label className="fp-check">
                ...
                <span>{typeof t('planner.snapGrid') === 'string' ? t('planner.snapGrid') : 'Grid snap'}</span>
              </label>
              ...
              <label className="fp-check" style={{ marginTop: 10 }}>
                ...
                <span>{typeof t('planner.snapAlign') === 'string' ? t('planner.snapAlign') : 'Wall / room snap'}</span>
              </label>

              <div className="fp-muted" style={{ marginTop: 8 }}>
                {typeof t('planner.snapHint') === 'string'
                  ? t('planner.snapHint')
            ...
            <div className="fp-section">
              ...
              {selectedRoom && (
                <div className="fp-note">
                  ...
                  {!selectedRoom.locked && (
                    <>
                      <label className="fp-label">{typeof t('planner.name') === 'string' ? t('planner.name') : 'Name'}</label>
                      <input
                        className="fp-input"
          ...
          <div className="fp-canvas-head">
            <div>
              ...
              <div className="fp-muted">
                ...
                {typeof t('planner.grid') === 'string' ? t('planner.grid') : 'Grid'}: {gridM} {unitM} ·{' '}
                {snapStatusText}
              </div>
            </div>
