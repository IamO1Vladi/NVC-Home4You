...
export default function Lightbox({ title, images, index=0, onClose, price, currency='EUR', desc, modelId, onRequest }){
  ...
  // ✅ Sanitize + keep QuickBase rich-text formatting (HTML)
  const safeDescHtml = useMemo(() => {
    if (!desc) return ''

    const raw = String(desc)

    // If desc is plain text (no HTML), preserve line breaks
    ...
    return DOMPurify.sanitize(html, {
      ...
      // Optional: keep design consistent (QuickBase sometimes injects inline styles)
      // If you want to keep inline styles, remove the next line:
      FORBID_ATTR: ['style'],
    })
  }, [desc])
        ...
        {(price || desc) && (
          <div className="lb-meta">
            ...
            <div style={{textAlign:'right'}}>
              ...
              <button
                className="btn mt-3"
                onClick={() => onRequest?.({ id: modelId, title })}
              >
                {t('product.request')}
              </button>
            </div>
