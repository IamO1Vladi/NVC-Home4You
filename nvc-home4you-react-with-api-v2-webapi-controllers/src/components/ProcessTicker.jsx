 ...
 * - NEW: media[] renders an image that fades with each step
...
export default function ProcessTicker({
  ...
  steps = [],
  media = [],               // [{src, alt}]
  intervalMs = 3200,        // ↑ slightly longer by default
  startPaused = false,
}) {
  ...
  // Preload media to avoid flicker
  useEffect(() => {
    media?.forEach(m => { if (m?.src) { const im = new Image(); im.src = m.src } })
  }, [media])

  // Autoplay loop
  useEffect(() => {
    if (reduceMotion || !playing || steps.length < 2) return
    const t = setInterval(() => setActive(i => (i + 1) % steps.length), intervalMs)
  ...
  // Keyboard control
  const onKeyDown = (e) => {
    if (e.key === 'ArrowRight') { setActive(i => Math.min(i + 1, steps.length - 1)); setPlaying(false) }
    if (e.key === 'ArrowLeft')  { setActive(i => Math.max(i - 1, 0)); setPlaying(false) }
    if (e.code === 'Space')     { setPlaying(p => !p); e.preventDefault() }
  }
  ...
  return (
    <section className="ptk" ref={rootRef} aria-labelledby="ptk-title" onKeyDown={onKeyDown}>
      ...
      {/* Step chips */}
      <div className="ptk-steps" role="tablist" aria-label="Process steps">
        {steps.map((label, i) => (
          <button
            key={i}
            role="tab"
      ...
      {/* NEW: Step media below with fade */}
      {hasMedia && (
        <div className="ptk-media" aria-label="Process illustration">
            ...
            {media.map((m, i) => (
              ...
              >
                <img
                  ...
                  onError={(e)=>{ e.currentTarget.src = `${import.meta.env.BASE_URL}modular-builds/card.svg` }}
                  width="1600"
                  height="900"
                  loading="lazy"
                />
