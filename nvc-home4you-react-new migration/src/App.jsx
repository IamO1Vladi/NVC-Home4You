...
function AppShell() {
  ...
  const { lang, setLang } = useI18n()
  const currentLocale = getLocaleFromPath(location.pathname) || (String(lang).toLowerCase().startsWith('bg') ? 'bg' : 'en')
  ...
  const trackRequestQuote = useCallback((payload) => {
   ...
   if (typeof window.fbq === 'function') {
    ...
    if (payload.modelId) {
      metaPayload.content_ids = [String(payload.modelId)]
  ...
  const submitOffer = useCallback(async (e) => {
    ...
    trackRequestQuote(payload)
    showToast('success', ui.common.toast.offerSuccess)
    setOfferOpen(false)
  }, [showToast, trackRequestQuote, ui.common.toast.offerError, ui.common.toast.offerSuccess])
  ...
  const homeRedirect = currentLocale === 'bg' ? paths.home.bg : paths.home.en
  const modularBuildsRedirect = currentLocale === 'bg' ? paths.modularBuilds.bg : paths.modularBuilds.en
  const modularHousesRedirect = currentLocale === 'bg' ? paths.modularHouses.bg : paths.modularHouses.en
  const faqRedirect = currentLocale === 'bg' ? paths.faq.bg : paths.faq.en
  const aboutRedirect = currentLocale === 'bg' ? paths.about.bg : paths.about.en
  const steelHousesRedirect = currentLocale === 'bg' ? paths.steelHouses.bg : paths.steelHouses.en
        ...
        <LocalePathGate>
          ...
          <Suspense fallback={<div className="page-loading" />}>
            <Routes>
              ...
              <Route path="/gallery" element={<Navigate to={String(lang).toLowerCase().startsWith('bg') ? '/bg/galeriq' : '/en/gallery'} replace />} />
<Route
  path="/bg/galeriq/*"
  element={
    <BgGalleryRoute
