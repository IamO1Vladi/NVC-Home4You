...
export default function BoxHouseConfiguratorPage({ content }) {
  ...
  const labels = React.useMemo(() => ({
    ...
    panoramic: t.labels?.panoramic || (isBg ? 'Френски / панорамни прозорци' : 'French / panoramic windows'),
    panoramicOpenable: t.labels?.panoramicOpenable || (isBg ? 'Отваряеми прозорци' : 'Openable windows'),
    panoramicFixed: t.labels?.panoramicFixed || (isBg ? 'Фиксирани прозорци' : 'Fixed windows'),
    panoramicNotes: t.labels?.panoramicNotes || (isBg ? 'Бележки за панорамните отвори' : 'Panoramic opening notes'),
    panoramicNotesPlaceholder: t.labels?.panoramicNotesPlaceholder || (isBg ? 'Пример: дневна отпред, спалня 1 отстрани, комплект към терасата' : 'Example: living room front, bedroom 1 side elevation, terrace door set'),
    ...
    panoramicNotesLabel: t.labels?.panoramicNotesLabel || (isBg ? 'Бележки за панорамните прозорци' : 'Panoramic notes'),
  ...
  const pdfText = React.useMemo(() => ({
    ...
    note: t.pdf?.note || (isBg ? 'Панорамните прозорци, вътрешните панели и вътрешните врати са включени в общата цена, когато са избрани. Електрическите работи по контактите остават за офериране.' : 'Panoramic windows, interior panel upgrades and inside doors are included in the known total when selected. Socket works remain quotation items.'),
  ...
  const panoramicPrice = (Number(config.panoramicOpenable || 0) * catalog.pricing.frenchWindowOpenable)
    + (Number(config.panoramicFixed || 0) * catalog.pricing.frenchWindowFixed)
  const insideDoorPrice = Number(config.insideDoorCount || 0) * catalog.pricing.insideDoorPerDoor
  const knownTotal = knownBasePrice + interiorPanelsPrice + heatingPrice + panoramicPrice + insideDoorPrice
  ...
  const summaryLines = React.useMemo(() => {
    const lines = [
      ...
      `${labels.panoramic}: ${config.panoramicOpenable || 0} / ${config.panoramicFixed || 0}${panoramicPrice ? ` (${euro(panoramicPrice, locale)})` : ''}`,
      config.panoramicNotes ? `${labels.panoramicNotesLabel}: ${config.panoramicNotes}` : '',
    ...
    config.panoramicFixed,
    config.panoramicNotes,
    config.panoramicOpenable,
    config.socketNotes,
  ...
  function addSocketMarker(point) {
    setConfig((prev) => ({
      ...
      sockets: [
        ...prev.sockets,
        {
          id: `${Date.now()}-${prev.sockets.length}`,
          x: point.x,
          y: point.y,
        },
      ],
  ...
  function exportPdf() {
    ...
    const priceRows = [
      ...
      [labels.panoramic, panoramicPrice ? euro(panoramicPrice, locale) : '-'],
      [labels.totalKnown, euro(knownTotal, locale)],
    ]
    ...
    const socketLegendHtml = socketMarkerItems.length
      ? socketMarkerItems.map((item) => `<span class="chip chip-dark">${escapeHtml(`${item.label} • ${item.coords}`)}</span>`).join('')
    ...
    <div class="overview">
      ...
      <div class="panel span-2">
        ...
        <div class="chips" style="margin-top:14px">
          ...
          <span class="chip">${escapeHtml(`${labels.panoramic}: ${config.panoramicOpenable || 0} / ${config.panoramicFixed || 0}`)}</span>
        </div>
        ${config.panoramicNotes ? `<div class="note"><div class="note-k">${escapeHtml(labels.panoramicNotesLabel)}</div><div class="note-v">${multilineHtml(config.panoramicNotes)}</div></div>` : ''}
      ...
      <>
        ...
        <div className="bhc-grid bhc-grid--2 bhc-grid--stack-mobile">
          <div className="bhc-stack">
            ...
            <div className="bhc-group">
              <div className="bhc-section-title">{labels.panoramic}</div>
              <div className="bhc-number-grid">
                <NumberField label={labels.panoramicOpenable} value={config.panoramicOpenable} onChange={(value) => setField('panoramicOpenable', value)} />
                <NumberField label={labels.panoramicFixed} value={config.panoramicFixed} onChange={(value) => setField('panoramicFixed', value)} />
              </div>
              <textarea value={config.panoramicNotes} onChange={(e) => setField('panoramicNotes', e.target.value)} placeholder={labels.panoramicNotesPlaceholder} rows={3} />
            </div>
          ...
          <div className="bhc-side-panel bhc-side-panel--sticky">
            ...
            <div className="bhc-picked-list">
              ...
              <SummaryRow label={labels.panoramic} value={`${config.panoramicOpenable || 0} / ${config.panoramicFixed || 0}`} />
    ...
    return (
      <>
        ...
        <div className="bhc-side-panel">
          ...
          <textarea value={config.socketNotes} onChange={(e) => setField('socketNotes', e.target.value)} placeholder={labels.socketNotesPlaceholder} rows={5} />
          ...
          <div className="bhc-chip-cloud">
            {socketMarkerItems.length ? socketMarkerItems.map((item) => (
              <span key={item.id} className="bhc-mini-chip">{item.label} • {item.coords}</span>
        ...
        <div className="bhc-summary-hero">
          ...
          <div className="bhc-summary-hero-copy">
            ...
            <div className="bhc-summary-actions">
              ...
              <button className="btn ghost" type="button" onClick={handleOpenOffer}>{actions.offer}</button>
          ...
          <div className="bhc-summary-board">
            ...
            <div className="bhc-side-panel bhc-side-panel--span-2">
              ...
              <div className="bhc-chip-cloud">
                ...
                <span className="bhc-mini-chip">{labels.panoramic}: {config.panoramicOpenable || 0} / {config.panoramicFixed || 0}</span>
              </div>
              {config.panoramicNotes ? (
                <div className="bhc-note-box">
                  <div className="bhc-note-title">{labels.panoramicNotesLabel}</div>
                  <div>{config.panoramicNotes}</div>
                </div>
              ) : null}
            ...
            <div className="bhc-side-panel bhc-side-panel--span-2">
              ...
              <div className="bhc-detail-list">
                ...
                <SummaryRow label={labels.insideDoorPrice} value={insideDoorPrice ? euro(insideDoorPrice, locale) : '-'} />
                <SummaryRow label={labels.heatingPrice} value={config.heating ? euro(heatingPrice, locale) : '-'} />
                <SummaryRow label={labels.panoramic} value={panoramicPrice ? euro(panoramicPrice, locale) : '-'} />
                <SummaryRow label={labels.totalKnown} value={euro(knownTotal, locale)} strong />
              </div>
              <div className="bhc-small-note">{labels.pricingFootnote}</div>
            </div>
