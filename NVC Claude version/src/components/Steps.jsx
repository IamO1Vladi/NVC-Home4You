      ...
      <div className="container">
        <div className="card p-6">
          ...
          <div className="progress mt-3" aria-hidden="true">
            <m.div initial={{ width: '16%' }} animate={{ width: `${pct}%` }} transition={{ type: 'spring', stiffness: 250, damping: 30 }} />
          </div>

          <div className="steps-grid mt-6">
            {items.map((item, i) => (
              <m.button key={item.title} className={`step${i === active ? ' active' : ''}`} onClick={() => setActive(i)} whileTap={{ scale: 0.99 }} layout>
