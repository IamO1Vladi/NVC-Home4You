...
function getFocusable(root) {
  return root
    ? Array.from(root.querySelectorAll('a[href], button, textarea, input, select, [tabindex]:not([tabindex="-1"])')).filter((el) => !el.hasAttribute('disabled'))
    : []
}
    ...
    <AnimatePresence>
      {open && (
        <div className="modal-portal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
          <m.div className="backdrop" onClick={onClose} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} />
          <m.div
            ref={ref}
          ...
          >
            <button className="close-x" aria-label={closeLabel} onClick={onClose}>✖</button>
