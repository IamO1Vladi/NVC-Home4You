...
const ROWS = [
  {
    id: 1, spentAt: '2026-08-14', categoryKey: 'transport-fuel', amount: 120.5, vatAmount: 20.08,
...
const ROLLUP = {
  byCategory: [
    { key: 'transport-fuel', total: 120.5, count: 1 },
...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn((url, options = {}) => {
    ...
    if (u.includes('/api/admin/operating-expenses/categories')) {
      return json(['salaries', 'rent', 'transport-fuel', 'marketing', 'other'])
    }
...
describe('AdminExpensesPage', () => {
  ...
  it('keeps the date and category after a save, clearing only the money fields', async () => {
    ...
    await user.selectOptions(category, 'transport-fuel')
    ...
    await waitFor(() => {
      ...
      expect(JSON.parse(post.body)).toMatchObject({
        spentAt: '2026-08-14', categoryKey: 'transport-fuel', amount: 120.5, vatAmount: null,
      })
    })
    ...
    expect(category.value).toBe('transport-fuel')
  })
